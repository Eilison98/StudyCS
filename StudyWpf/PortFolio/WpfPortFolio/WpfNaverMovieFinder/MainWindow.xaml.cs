...
namespace WpfMahAppTest
{
    ...
    {
        bool IsFavorite = false;  //  네이버 Api로 검색한건지, 즐겨찾기 DB에서 가져온것인지 확인할 값
        //   IsFavorite = True -> DB에서
        //   IsFavorite = false -> 네이버 Api
        ...
        private void btnSearch_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            {
                stsResult.Content = "검색할 영화 명을 입력, 검색버튼을 클릭하세요.";
                // MessageBox.Show("검색할 영화 명을 입력, 검색버튼을 클릭하세요.");
                Commons.ShowMessageAsync("검색", "검색할 영화 명을 입력, 검색버튼을 클릭하세요.");
                return;
            }
        ...
        {
            string clientID = "RImPWJOI7yFtqATdD0rS";
            string clientSecret = "BTXjq8HmoO";
            ...
            try
            {
                ...
                request.Headers.Add("X-Naver-Client-Secret", clientSecret);  //  중요 네이버 API요청문구 정해져 있음
            ...
            var parsedJson = JObject.Parse(result);  //  string ti json
        ...
        private void btnViewWatchList_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                IsFavorite = true;  //  DB에서 가져왔으므로 true 
            }
            catch (Exception ex)
            {
                Commons.ShowMessageAsync("예외", $"예외발생 : {ex}");
        ...
        private void btnDelWatchList_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            //btnViewWatchList_Click(sender, e);  //  즐겨찾기보기 버튼클릭 이벤트를 실행
            this.DataContext = null;
            txtSearchName.Text = string.Empty;
        ...
        /// <summary>
        /// 유튜브 예고편 동영상보기 이벤트 헨들러
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnWatchTrailerList_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            var trailerWindow = new TrailerWindow(movieName);  //  영화제목 받는 생성자로 변경!
            trailerWindow.Owner = this; //  MainWindow
            trailerWindow.WindowStartupLocation = WindowStartupLocation.CenterOwner;  //  MainWindow Center에 화면을 띄우겠다.
        ...
        {
            if(grdResult.SelectedItem is MovieItem)  //  네이버 Api에서 온값이면
            {
                var movie = grdResult.SelectedItem as MovieItem;  //  타입 변환
                if (string.IsNullOrEmpty(movie.Image))
                {
                    imgPoster.Source = new BitmapImage(new Uri("/resource/No_P.jpg", UriKind.RelativeOrAbsolute));
                }
                else
                {
